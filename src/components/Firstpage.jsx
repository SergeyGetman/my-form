...
const Firstpage = () => {
  ...
  return (
    <div className={cl.page_title}>
      ...
      <button className={cl.fourth} onClick={checkCity}>
        Find Town
      </button>
      <input type="text" />
      <br />
      <div>{`this is number ${numberOfAction} `}</div>
      <button className={cl.fourth} onClick={numberOfActionTake}>
        Give the number
      </button>
      <button className={cl.fourth} onClick={numberMinus}>
        Minus the number
      </button>

      <div>{`this is new town ${seaTown}`}</div>
      <br />
      <div>
        {`this is ${number} ${age} ${name}`}
        <button className={cl.fourth} onClick={getInformation}>
          GET Information
        </button>
      </div>
      <br />
      ...
      <form onSubmit={handleSubmit(onSubmit2)}>
        ...
        <div>
          ...
          <input placeholder="age" {...register('age', { onChange: true })} />
        </div>
